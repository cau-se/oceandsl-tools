...
public class CreateCommonBlocksTableStage extends AbstractTransformation<CommonBlockEntry, Table> {
    ...
    protected void execute(final CommonBlockEntry entry) throws Exception {
        final String files = entry.modules.stream().map(module -> module.getFileName())
                .reduce((list, element) -> list += "," + element).get();
        final String modules = entry.modules.stream().map(module -> module.getModuleName())
                .reduce((list, element) -> list += "," + element).get();
        final String variables = entry.variables.stream().reduce((list, element) -> list += "," + element).get();
