 ...
 * @since 1.1
 */
public class CreateGraph4AllenMetric extends AbstractTransformation<ModelRepository, Graph<Node<DeployedOperation>>> {

    @Override
    protected void execute(final ModelRepository element) throws Exception {
        final Graph<Node<DeployedOperation>> graph = this.computeGraph(element);
        this.outputPort.send(graph);
    }

    private Graph<Node<DeployedOperation>> computeGraph(final ModelRepository repository) {
        final MutableGraph<Node<DeployedOperation>> graph = this.createGraph();
        ...
        for (final DeployedOperation operation : this.collectAllOperations(deploymentModel)) {
            graph.addNode(new Node<>(operation));
            this.getReferencedMembers(operation, executionModel)
                    .forEach(calledOperation -> graph.putEdge(new Node<>(calledOperation), new Node<>(operation)));
        }
        return graph;
    }
    ...
    private MutableGraph<Node<DeployedOperation>> createGraph() {
