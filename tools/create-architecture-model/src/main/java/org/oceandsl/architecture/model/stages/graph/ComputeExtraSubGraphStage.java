 ...
 *
 */
public class ComputeExtraSubGraphStage extends AbstractTransformation<ModelRepository, IGraph> {

    private final String subgraphName;

    public ComputeExtraSubGraphStage(final String subgraphName) {
    ...
    private boolean findInvocationInGraph(final SourceModel sourcesModel, final AggregatedInvocation invocation) {
        ...
        return (sources.contains(this.subgraphName) && targets.contains(this.subgraphName) && (sources.size() == 1)
                && (targets.size() == 1));
    }
