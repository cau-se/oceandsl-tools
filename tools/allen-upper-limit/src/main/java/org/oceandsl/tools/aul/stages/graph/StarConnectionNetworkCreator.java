 ...
 * @since 1.4
 */
public class StarConnectionNetworkCreator implements INetworkCreator {
    ...
    public void createEdges(final MutableGraph<Node<DeployedComponent>> graph,
        ...
        for (int i = 1; i < numOfNodes; i++) {
            final Node<DeployedComponent> source = nodes.get(0);
            final Node<DeployedComponent> target = nodes.get(i);
