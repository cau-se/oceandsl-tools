...
public class SimilarMethodSetMergeInterfaceStage extends AbstractMergeInterfaceStage {
    ...
    private boolean isContainedInList(final List<MethodDistance> list, final MethodDistance distance) {
        return list.stream().anyMatch(
                element -> element.firstIface == distance.firstIface && element.secondIface == distance.secondIface
...
//        final double distance = (this.countMissingMethods(firstIface, secondIface)
//                / firstIface.getProvidedOperationTypes().size()
//                + this.countMissingMethods(secondIface, firstIface) / secondIface.getProvidedOperationTypes().size())
//                / 2.0;
        final double da = this.countMissingMethods(firstIface, secondIface);
        final double db = this.countMissingMethods(secondIface, firstIface);
    ...
    private double countMissingMethods(final ProvidedInterfaceType firstIface,
            final ProvidedInterfaceType secondIface) {
